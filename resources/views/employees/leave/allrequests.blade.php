...
@section('content-box-title', 'Leave Requests')
@section('content-box-icon', 'fas fa-sign-out-alt')
@section('content-box')
	<table class="styled h-center" id="leave-history">
		...
		<tbody>
			@if (count($Leaves) === 0)
				<tr>
					<td colspan="7">No leaves to show</td>
				</tr>
			@else
				@foreach($Leaves as $Leave)
					<tr data-id="{{ $Leave -> id }}">
						<td>@component('partials.employee_profile') @slot('employee_id', $Leave -> employee_id) @slot('link', true) @endcomponent</td>
